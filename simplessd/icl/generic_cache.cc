...
void GenericCache::calcIOPosition(uint64_t key, uint32_t &row, uint32_t &col) {
  uint64_t lca = getKeyLCA(key);
...
uint32_t GenericCache::getValidWay(uint64_t key, uint64_t &tick) {
...
bool GenericCache::read(Request &req, uint64_t &tick) {
  ...
  if (useReadCaching) {
    ...
    if (wayIdx != waySize) {
      ...
      if (useReadPrefetch && reqKey == prefetchTrigger) {
        debugprint(LOG_ICL_GENERIC_CACHE, "READ  | Prefetch triggered");

        req.range.slpn = getKeyLCA(lastPrefetched);
        reqKey = lastPrefetched;
    ...
    ICL_GENERIC_CACHE_READ:
      ...
      for (uint64_t lca = beginLCA; lca < endLCA; lca++) {
        ...
        readList.push_back({key, ((uint64_t)setIdx << 32) | wayIdx});
  ...
  if (useWriteCaching) {
    ...
    if (wayIdx != waySize) {
      ...
      // We have to flush
        ...
        uint32_t row, col;  // Variable for I/O position (IOFlag)
        uint32_t setToFlush = calcSetIndex(reqKey);
