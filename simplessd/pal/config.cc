...
const char NAME_SLC_BLOCK[] = "SLCBlockPerPlane";
const char NAME_TLC_BLOCK[] = "TLCBlockPerPlane";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_SLC_BLOCK)) {
    slcBlock = strtoul(value, nullptr, 10);
  }
  else if (MATCH_NAME(NAME_TLC_BLOCK)) {
    tlcBlock = strtoul(value, nullptr, 10);
  }
...
void Config::update() {
  ...
  if (slcBlock + tlcBlock != block) {
    panic("SLCBlockPerPlane + TLCBlockPerPlane should equal Block.");
  }
