...
class PALOLD : public AbstractPAL {
 private:
  ...
  struct {
    ...
    uint64_t writeCount;
    uint64_t eraseCount;
    uint64_t slcReadCount;
    uint64_t slcWriteCount;
    uint64_t slcEraseCount;
    uint64_t tlcReadCount;
    uint64_t tlcWriteCount;
    uint64_t tlcEraseCount;
  } stat;

  void convertCPDPBP(Request &, std::vector<::CPDPBP> &);
  void selectLatency(::CPDPBP &);
