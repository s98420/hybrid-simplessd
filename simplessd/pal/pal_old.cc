...
PALOLD::PALOLD(Parameter &p, ConfigReader &c)
  ...
  slcBlock = conf.readUint(CONFIG_PAL, NAND_SLC_BLOCK);
  tlcBlock = conf.readUint(CONFIG_PAL, NAND_TLC_BLOCK);
...
void PALOLD::selectLatency(::CPDPBP &addr) {
  if (addr.Block >= param.block) {
    panic("pal: physical block index out of range");
  }

  // Validate the configured SLC/TLC physical partition. Latency selection
  // remains physical-block based, not host-tier based.
  if (addr.Block < slcBlock) {
    lat = latSLC;
  }
  else if (addr.Block < slcBlock + tlcBlock) {
    lat = latTLC;
  }
  else {
    panic("pal: decoded physical block is outside configured SLC/TLC "
          "partition");
  }
...
void PALOLD::getStatList(std::vector<Stats> &list, std::string prefix) {
  ...
  temp.name = prefix + "slc.read.count";
  temp.desc = "Total SLC-region read operation count";
  list.push_back(temp);

  temp.name = prefix + "slc.program.count";
  temp.desc = "Total SLC-region program operation count";
  list.push_back(temp);

  temp.name = prefix + "slc.erase.count";
  temp.desc = "Total SLC-region erase operation count";
  list.push_back(temp);

  temp.name = prefix + "tlc.read.count";
  temp.desc = "Total TLC-region read operation count";
  list.push_back(temp);

  temp.name = prefix + "tlc.program.count";
  temp.desc = "Total TLC-region program operation count";
  list.push_back(temp);

  temp.name = prefix + "tlc.erase.count";
  temp.desc = "Total TLC-region erase operation count";
  list.push_back(temp);
...
void PALOLD::getStatValues(std::vector<double> &values) {
  ...
  values.push_back(stat.slcReadCount);
  values.push_back(stat.slcWriteCount);
  values.push_back(stat.slcEraseCount);
  values.push_back(stat.tlcReadCount);
  values.push_back(stat.tlcWriteCount);
  values.push_back(stat.tlcEraseCount);
