...
const char NAME_SLC_FILL_RATIO[] = "SLCFillRatio";
const char NAME_TLC_FILL_RATIO[] = "TLCFillRatio";
const char NAME_SLC_INVALID_PAGE_RATIO[] = "SLCInvalidPageRatio";
const char NAME_TLC_INVALID_PAGE_RATIO[] = "TLCInvalidPageRatio";
...
Config::Config() {
  ...
  slcInvalidRatio = 0.f;
  tlcInvalidRatio = 0.f;
  slcFillingRatioSet = false;
  tlcFillingRatioSet = false;
  slcInvalidRatioSet = false;
  tlcInvalidRatioSet = false;
  reclaimBlock = 1;
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_SLC_FILL_RATIO)) {
    if (strlen(value) > 0) {
      slcFillingRatio = strtof(value, nullptr);
      slcFillingRatioSet = true;
    }
  }
  else if (MATCH_NAME(NAME_TLC_FILL_RATIO)) {
    if (strlen(value) > 0) {
      tlcFillingRatio = strtof(value, nullptr);
      tlcFillingRatioSet = true;
    }
  }
  else if (MATCH_NAME(NAME_SLC_INVALID_PAGE_RATIO)) {
    if (strlen(value) > 0) {
      slcInvalidRatio = strtof(value, nullptr);
      slcInvalidRatioSet = true;
    }
  }
  else if (MATCH_NAME(NAME_TLC_INVALID_PAGE_RATIO)) {
    if (strlen(value) > 0) {
      tlcInvalidRatio = strtof(value, nullptr);
      tlcInvalidRatioSet = true;
    }
  }
...
void Config::update() {
  ...
  if (!slcFillingRatioSet) {
    slcFillingRatio = fillingRatio;
  }
  if (!tlcFillingRatioSet) {
    tlcFillingRatio = fillingRatio;
  }
  if (!slcInvalidRatioSet) {
    slcInvalidRatio = invalidRatio;
  }
  if (!tlcInvalidRatioSet) {
    tlcInvalidRatio = invalidRatio;
  }

  if (slcFillingRatio < 0.f || slcFillingRatio > 1.f) {
    panic("Invalid SLCFillRatio");
  }
  if (tlcFillingRatio < 0.f || tlcFillingRatio > 1.f) {
    panic("Invalid TLCFillRatio");
  }
  if (slcInvalidRatio < 0.f || slcInvalidRatio > 1.f) {
    panic("Invalid SLCInvalidPageRatio");
  }
  if (tlcInvalidRatio < 0.f || tlcInvalidRatio > 1.f) {
    panic("Invalid TLCInvalidPageRatio");
  }
}
...
float Config::readFloat(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case FTL_INVALID_PAGE_RATIO:
      ret = invalidRatio;
      break;
    case FTL_SLC_FILL_RATIO:
      ret = slcFillingRatio;
      break;
    case FTL_TLC_FILL_RATIO:
      ret = tlcFillingRatio;
      break;
    case FTL_SLC_INVALID_PAGE_RATIO:
      ret = slcInvalidRatio;
      break;
    case FTL_TLC_INVALID_PAGE_RATIO:
      ret = tlcInvalidRatio;
      break;
