...
static Tier parseTier(uint32_t value) {
  if (value & ~SIM_NVME_TIER_MASK) {
    panic("Invalid NVMe tier flag");
  }

  value = (value & SIM_NVME_TIER_MASK) >> SIM_NVME_TIER_SHIFT;
...
void Namespace::flush(SQEntryWrapper &req, RequestFunction &func) {
  ...
  CQEntryWrapper resp(req);
  // Simulator extension: dword13 tier selects the logical space to flush.
  // SLC flushes SLC-local LBAs; TLC flushes TLC-local LBAs.
  Tier tier = parseTier(req.entry.dword13);
  ...
  if (!err) {
    DMAFunction begin = [this](uint64_t, void *context) {
      ...
      pParent->flush(this, pContext->tier, doFlush, context);
    };
...
void Namespace::write(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    DMAFunction doRead = [this](uint64_t tick, void *context) {
      ...
      pParent->write(this, pContext->slba, pContext->nlb, pContext->tier,
                     dmaDone, context);
    };
...
void Namespace::read(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    DMAFunction doRead = [this](uint64_t tick, void *context) {
      ...
      pParent->read(this, pContext->slba, pContext->nlb, pContext->tier,
                    dmaDone, pContext);
...
void Namespace::compare(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    pContext->slba = slba;
    pContext->nlb = nlb;
    pContext->tier = parseTier(req.entry.dword13);
    ...
    DMAFunction doTrim = [this](uint64_t, void *context) {
      DMAFunction dmaDone = [this](uint64_t, void *context) {
        ...
        for (uint64_t i = 0; i < pContext->slba; i++) {
          ...
          pDMA->counter++;
          pParent->trim(this, range.slba, range.nlb, pContext->tier,
                        eachTrimDone, pDMA);
        }
