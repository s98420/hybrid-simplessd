...
void Subsystem::read(Namespace *ns, uint64_t slba, uint64_t nlblk, Tier tier,
  ...
  convertUnit(ns, slba, nlblk, tier, *req);

  execute(CPU::NVME__SUBSYSTEM, CPU::CONVERT_UNIT, doRead, req);
}

void Subsystem::write(Namespace *ns, uint64_t slba, uint64_t nlblk, Tier tier,
                      DMAFunction &func, void *context) {
  Request *req = new Request(func, context);
...
void Subsystem::flush(Namespace *ns, Tier tier, DMAFunction &func,
                      void *context) {
  ...
  req.range.nlp = tierLogicalPages;
  req.range.tier = tier;
  req.tier = tier;
  req.offset = 0;
  req.length = tierLogicalPages * tierLogicalPageSize;

  pHIL->flush(req);
}

void Subsystem::trim(Namespace *ns, uint64_t slba, uint64_t nlblk, Tier tier,
                     DMAFunction &func, void *context) {
  Request *req = new Request(func, context);
...
void Subsystem::migrate(Namespace *ns, MigrationRequest &req,
                        DMAFunction &func, void *context) {
  struct MigrationContext {
    ...
    MigrationContext(MigrationRequest &r, DMAFunction &f, void *c)
  ...
  DMAFunction doMigrate = [this](uint64_t, void *context) {
    ...
    pHIL->migrate(*pContext->req, pContext->function, pContext->context);

    delete pContext;
  };

  execute(CPU::NVME__SUBSYSTEM, CPU::CONVERT_UNIT, doMigrate, pContext);
}
