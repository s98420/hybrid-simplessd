...
void HIL::migrate(MigrationRequest &req, DMAFunction &func, void *context) {
  ...
  execute(CPU::HIL, CPU::FLUSH, doMigrate,
          new MigrationContext(req, func, context));
}
...
void HIL::getTierLPNInfo(Tier tier, uint64_t &totalLogicalPages,
                         uint32_t &logicalPageSize) {
  pICL->getTierLPNInfo(tier, totalLogicalPages, logicalPageSize);
}
