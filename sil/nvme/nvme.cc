...
void Driver::getInfo(uint8_t tier, uint64_t &bytesize, uint32_t &minbs) {
  uint64_t blocks =
      conf.readUint(SimpleSSD::CONFIG_PAL, SimpleSSD::PAL::NAND_BLOCK);
  uint64_t tierBlocks = 0;

  if (tier == 0) {
    tierBlocks =
        conf.readUint(SimpleSSD::CONFIG_PAL, SimpleSSD::PAL::NAND_SLC_BLOCK);
  }
  else if (tier == 1) {
    tierBlocks =
        conf.readUint(SimpleSSD::CONFIG_PAL, SimpleSSD::PAL::NAND_TLC_BLOCK);
  }
  else {
    SimpleSSD::panic("Invalid tier");
  }

  bytesize = capacity * tierBlocks / blocks;
  minbs = LBAsize;
}
...
void Driver::submitIO(BIL::BIO &bio) {
  ...
  cmd[13] = ((uint32_t)bio.tier << SimpleSSD::HIL::NVMe::SIM_NVME_TIER_SHIFT) &
            SimpleSSD::HIL::NVMe::SIM_NVME_TIER_MASK;
