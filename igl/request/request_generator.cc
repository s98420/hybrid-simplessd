...
RequestGenerator::RequestGenerator(Engine &e, BIL::BlockIOEntry &b,
  ...
  defaultTier = (uint8_t)c.readUint(CONFIG_REQ_GEN, REQUEST_DEFAULT_TIER);
  enableMigration =
      c.readBoolean(CONFIG_REQ_GEN, REQUEST_ENABLE_MIGRATION);
...
void RequestGenerator::init(uint64_t bytesize, uint32_t bs, uint64_t slcBytes,
                            uint64_t tlcBytes) {
  bytesize = defaultTier == 0 ? slcBytes : tlcBytes;
  logicalBlockSize = bs;
  tierLBA[0] = slcBytes / bs;
  tierLBA[1] = tlcBytes / bs;

  if (offset > bytesize) {
    SimpleSSD::panic("offset is larger than selected tier size");
  }
  if (blocksize < bs) {
...
void RequestGenerator::printStats(std::ostream &out) {
  ...
  out << "Migration (counts): generated=" << migration_count
      << ", success=" << migration_success_count
      << ", failed=" << migration_failed_count
      << ", skipped=" << migration_skipped_count << std::endl;
...
bool RequestGenerator::rangeValid(uint8_t tier, uint64_t lba, uint64_t nlb) {
...
void RequestGenerator::markRange(uint8_t tier, uint64_t lba, uint64_t nlb,
  ...
  for (uint64_t i = 0; i < nlb; i++) {
    if (valid) {
      validLBA[tier].insert(lba + i);
    }
    else {
      validLBA[tier].erase(lba + i);
    }
  }
}

bool RequestGenerator::generateMigration(BIL::BIO &bio) {
  uint8_t srcTier = 0;
  uint8_t dstTier = 1;
  uint64_t nlb = blocksize / logicalBlockSize;

  if (!enableMigration || migrationRatio <= 0.f ||
      ratioGen(randengine) >= migrationRatio || nlb == 0) {
    return false;
  }

  if (migrationDirection == MIGRATION_TLC_TO_SLC ||
      (migrationDirection == MIGRATION_BOTH && ratioGen(randengine) >= 0.5f)) {
    srcTier = 1;
    dstTier = 0;
  }
...
void RequestGenerator::_submitIO(uint64_t) {
  ...
  bio.id = io_count++;
  bio.tier = defaultTier;
  bio.source = BIL::BIO_SOURCE_GENERATOR;

  isMigration = generateMigration(bio);

  // This function also uses io_count (=1 at very beginning)
  if (!isMigration && nextIOIsRead()) {
    bio.type = BIL::BIO_READ;
    read_count++;
  }
  else if (!isMigration) {
    bio.type = BIL::BIO_WRITE;
    markRange(defaultTier, bio.offset / logicalBlockSize,
              DIVCEIL(bio.length, logicalBlockSize), true);
  }
  ...
  if (isMigration) {
    pendingMigration.emplace(bio.id, bio);
  }
...
void RequestGenerator::_iocallback(uint64_t id, uint16_t status) {
  ...
  auto iter = pendingMigration.find(id);
