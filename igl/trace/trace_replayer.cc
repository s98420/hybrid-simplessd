...
TraceReplayer::TraceReplayer(Engine &e, BIL::BlockIOEntry &b,
  ...
  defaultTier = (uint8_t)c.readUint(CONFIG_TRACE, TRACE_DEFAULT_TIER);
...
void TraceReplayer::init(uint64_t bytesize, uint32_t bs, uint64_t slcBytes,
                         uint64_t tlcBytes) {
  ssdSize = bytesize;
  tierSize[0] = slcBytes;
  tierSize[1] = tlcBytes;
...
uint8_t TraceReplayer::getTier(std::string tier) {
  ...
  uint64_t value = strtoul(tier.c_str(), nullptr, useHex ? 16 : 10);

  if (value > 1) {
    SimpleSSD::panic("Unexpected tier value.");
  }
...
void TraceReplayer::parseLine() {
  ...
  if (groupID[ID_TIER] > 0) {
    if (match.size() <= groupID[ID_TIER]) {
      SimpleSSD::panic("Tier parse failed");
    }
...
void TraceReplayer::submitIO() {
  ...
  bio.source = BIL::BIO_SOURCE_TRACE;
