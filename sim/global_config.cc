...
const char NAME_REQUEST_LATENCY_LOG_ENABLE[] = "EnableRequestLatencyLog";
const char NAME_REQUEST_LATENCY_LOG_FILE[] = "RequestLatencyLogFile";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_REQUEST_LATENCY_LOG_ENABLE)) {
    requestLatencyLogEnable = convertBoolean(value);
  }
  else if (MATCH_NAME(NAME_REQUEST_LATENCY_LOG_FILE)) {
    requestLatencyLogFile = value;
  }
...
uint64_t Config::readUint(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case GLOBAL_COMPLETION_LATENCY:
      ...
      break;
    case GLOBAL_REQUEST_LATENCY_LOG_ENABLE:
      ret = requestLatencyLogEnable;
      break;
  }
