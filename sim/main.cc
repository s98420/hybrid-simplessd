...
std::ostream *pRequestLatencyFile = nullptr;
...
std::ofstream requestLatencyFile;
...
int main(int argc, char *argv[]) {
  ...
  std::string requestLatencyLogPath =
      simConfig.readString(CONFIG_GLOBAL, GLOBAL_REQUEST_LATENCY_LOG_FILE);
  ...
  if (simConfig.readUint(CONFIG_GLOBAL, GLOBAL_REQUEST_LATENCY_LOG_ENABLE)) {
    std::string full(argv[3]);

    joinPath(full, requestLatencyLogPath);
    requestLatencyFile.open(full);

    if (!requestLatencyFile.is_open()) {
      std::cerr << " Failed to open log file: " << full << std::endl;

      return 3;
    }

    pRequestLatencyFile = &requestLatencyFile;
  }
  ...
  std::function<void()> beginCallback = []() {
    uint64_t bytesize;
    uint64_t slcBytesize;
    uint64_t tlcBytesize;
    uint32_t bs;

    pInterface->getInfo(bytesize, bs);
    pInterface->getInfo(0, slcBytesize, bs);
    pInterface->getInfo(1, tlcBytesize, bs);
    pIOGen->init(bytesize, bs, slcBytesize, tlcBytesize);
    pIOGen->begin();
  };
